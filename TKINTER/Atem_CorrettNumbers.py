...
def PGM_and_PVW_has_chaned():
    ...
    global last_PGW_MINI
    while not stop_flag:
        last_PGM4K = atem4K.programInput[1].videoSource.value
        last_PVW4K = atem4K.previewInput[1].videoSource.value
        ...
        if last_PVW4K != PVW or last_PGM4K != PGM:
            PVW = last_PVW4K
            PGM = last_PGM4K
            setPGMandPVWtoCurrent()
            consoleText()
...
if PGM == 5:
    PGM = atem4K.programInput[1].videoSource.value + atemMini.programInput[0].videoSource.value
